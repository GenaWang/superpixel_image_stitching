...
void findFeaturePointsWithSIFT(Mat img_1, Mat img_2, vector<KeyPoint>& key_p1, vector<KeyPoint>& key_p2, vector<DMatch>& matches) {
	...
	clock_t t1, t2;
	t1 = clock();
	vector<vector<KeyPoint>> key_p;
	key_p.resize(2);
	vector<Mat> des;
	des.resize(2);
#pragma omp parallel for
	for (int i = 0;i < 2;i++) {
		f2d->detect(img_1, key_p[i]);
	}

	key_p1.resize(key_p[0].size());
	key_p2.resize(key_p[1].size());

#pragma omp parallel for
	for (int i = 0;i < key_p[0].size();i++) {
		key_p1[i] = key_p[0][i];
	}
#pragma omp parallel for
	for (int i = 0;i < key_p[1].size();i++) {
		key_p2[i] = key_p[1][i];
	}
	...
	*/
	...
	//SIFT特征匹配：Matching descriptor vector using FlannBasedMatcher（选择性匹配方式）
	FlannBasedMatcher matcher;
	vector<vector<DMatch> > matchePoints;
	vector<Mat> train_desc(1, des2);
	matcher.add(train_desc);
	matcher.train();

	matcher.knnMatch(des1, matchePoints, 2);
	...
	Mat first_match;
	drawMatches(img_1, key_p1, img_2, key_p2, matches, first_match);
	imwrite("FirstGoodMatch.jpg", first_match);

}

void findFeaturePointsWithSIFT(vector<Mat> img, vector<vector<KeyPoint>>& key_p, vector<DMatch>& matches) {
	vector<Mat> des;
	des.resize(img.size());
	key_p.resize(img.size());

	//SIFT寻找特征点
	Ptr<Feature2D> f2d = xfeatures2d::SIFT::create();
#pragma omp parallel for
	for (int i = 0;i < 2;i++) {
		f2d->detect(img[i], key_p[i]);
		f2d->compute(img[i], key_p[i], des[i]);
	}

	//SIFT特征匹配：Matching descriptor vector using FlannBasedMatcher（选择性匹配方式）
	FlannBasedMatcher matcher;
	vector<vector<DMatch> > matchePoints;
	vector<Mat> train_desc(1, des[1]);
	matcher.add(train_desc);
	matcher.train();

	matcher.knnMatch(des[0], matchePoints, 2);
	for (int i = 0; i < matchePoints.size(); i++)
	{
		if (matchePoints[i][0].distance < 0.8 * matchePoints[i][1].distance)
		{
	...
	Mat first_match;
	drawMatches(img[0], key_p[0], img[1], key_p[1], matches, first_match);
	imwrite("FirstGoodMatch.jpg", first_match);
