...
cv::Mat MBS::Visualization1() {
	vector<vector<int>> pixelLabels;
	pixelLabels.resize(_spCnt);
	for (int i = 0; i < _imgWidth*_imgHeight; i++)
		pixelLabels[_labels[i]].push_back(i);
	cv::Mat visImg(_imgHeight, _imgWidth, CV_8UC3);
	cv::Vec3b color;
	srand(0);
	for (int i = 0; i < _spCnt; i++) {
		...
		color[1] = rand() % 200 + 50;
		color[2] = rand() % 200 + 50;
		for (int j = 0; j < pixelLabels[i].size(); j++) {
			int x = pixelLabels[i][j] / _imgWidth;
			int y = pixelLabels[i][j] - x*_imgWidth;
			// cout << x << " " << y << endl;
			visImg.at<cv::Vec3b>(x, y) = color;
		}
	}
...
//created by WangJin
//*************************
void MBS::getSuperPixels(vector<vector<pair<int, int>>>& superPixels) {
	superPixels.resize(_spCnt);
	...
	for (int i = 0; i < _imgWidth*_imgHeight; i++) {
		...
		// int y = i - x*_imgWidth;
		int x = i%_imgWidth;
		superPixels[_labels[i]].push_back(make_pair(x, y));
	}
	return;
}

int MBS::computeSuperPixelsSize(vector<vector<pair<int, int>>>& superPixels) {
	int count = 0;
...
//created by WangJin
//*************************
void MBS::computeSuperPixelCenter(vector<vector<pair<int, int>>>& superPixels, vector<pair<int, int>>& superpixelsCenter){
	superpixelsCenter.resize(_spCnt);
	for (int i = 0; i < _spCnt; i++) {
		pair<int, int> centor = computePointsCenter(superPixels[i]);
		superpixelsCenter[i] = centor;
	}
	return;
}
...
//created by WangJin
//*************************
pair<int, int> MBS::computePointsCenter(vector<pair<int, int>>& points) {
	pair<int, int> centor;
	int countPoints = points.size();
	double tempNum = 1.0 / double(countPoints);
	double pointsSumX = 0, pointsSumY = 0;
	for (int i = 0; i < points.size(); i++) {
		pointsSumX += points[i].first*tempNum;
		pointsSumY += points[i].second*tempNum;
	}
	centor.first = int(pointsSumX);
	centor.second = int(pointsSumY);
	return centor;
}
...
void dilateSuperPixel(int height, int weight, vector<pair<int, int>>& superPixel, vector<vector<bool>>& dilateStruct, vector<pair<int, int>>& dilatedSuperPixel) {
	...
	//注意1：此处flagMat的存储形式为 flagMat[y][x]，而不是 flagMat[x][y]
	//注意2：此处superPixel的存储形式为superPixel[x][y],而不是superPixel[y][x]
	//注意3：因此dilatedSuperPixel的存储形式应该是dilatedSuperPixel[x][y]

	//step1:Initialize the flagMat
	// flagMat.resize(height);
	for (int i = 0;i < height;i++) {
		flagMat.push_back(vector<bool>(weight, 0));
	}

	//step2:Assigned superPixel to flagMat
	for (int i = 0;i < superPixel.size();i++)
		flagMat[superPixel[i].second][superPixel[i].first] = 1;

	//step3:dialte the superPixel
	int structHeight = dilateStruct.size(), structWeight = dilateStruct[0].size();
	for (int i = 0;i < height;i++) {
		for (int j = 0;j < weight;j++) {
			int tempVal = 0;
			if (i < (height - structHeight) && j < (weight - structWeight)) {
				for (int m = 0;m < structHeight;m++) {
					for (int n = 0;n < structWeight;n++) {
						tempVal = tempVal || (flagMat[i + m][j + n] && dilateStruct[m][n]);
						// tempVal = tempVal || flagMat[i + m][j + n];
					}
				}
				flagMat[i][j] = tempVal;
			}
			...
			if (tempVal)
				dilatedSuperPixel.push_back(make_pair(j, i));
		}
	}
...
void MBS::dilateSuperPixels(vector<vector<pair<int, int>>>& superPixels, vector<vector<pair<int, int>>>& dilatedSuperPixels, vector<vector<bool>>& dilateStruct) {
	for (int i = 0;i < superPixels.size();i++) {
		vector<pair<int, int>> dilatedSuperPixel;
		dilateSuperPixel(_imgHeight, _imgWidth, superPixels[i], dilateStruct, dilatedSuperPixel);
		dilatedSuperPixels.push_back(dilatedSuperPixel);
	}
}